...
## R-10 Windows 전역 키보드 훅(pynput)이 기업용 EDR/백신에 의해 차단될 수 있다 (MEDIUM, OPEN)
...
- **완화 방안**
  1. 초기 설치 시 사용자에게 "키보드·마우스 활동 감지 권한 필요" 공지 및 방해 금지 모드 안내.
  2. pynput 실패 시 Windows `GetLastInputInfo()` API(pywin32)로 폴백 — 훅이 아닌 폴링 방식이라 EDR 탐지 가능성 낮음.
  3. IT 부서와 협의하여 본 프로세스의 바이너리 해시를 EDR 예외 목록에 등록.
- **담당**: M_10 스펙에서 폴백 구현을 필수 포함.
