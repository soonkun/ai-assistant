...
| D-4 | **WebSocket 메시지 타입: `avatar-state` 독립.** upstream `actions.expressions`에 얹지 않음. | 스프라이트 기반은 오디오와 감정 동기화 제약이 Live2D보다 약함. 독립 타입이 (a) 백엔드 emitter 경로(`push_event`)와 LLM 발화 경로 양쪽에서 재사용 가능(M_06 DocumentIngest, M_11 ProactiveDispatcher 등 어떤 호출자든 동일 타입으로 감정만 갱신), (b) 메시지 페이로드 작아 네트워크 효율적, (c) M_12가 단순 switch 처리 가능. |
...
7. **M_11 프로액티브 발화 이벤트와의 상호 순서 보장**: M_11은 단일 WS 송신 타입 `ai-speak-signal`을 사용(`specs/M_11_ProactiveDispatcher_SPEC.md` §7.3). 본 모듈은 `avatar-state` 이벤트와 M_11의 `ai-speak-signal` 송신 사이 상대적 순서를 보장하지 않는다(호출자인 M_11이 결정).
